  ...
  module Helpers
    module ActiveRecordHelper
      def error_messages_for(object_names, options = {})
        ...
        errors = []
        objects.each { |obj| obj.errors.each { |attr, val| errors << [l(attr), l(val)] } }
        ...
        unless errors.empty?
          ...
          content_tag("div",
            ...
            content_tag("ul", errors.collect { |value| content_tag("li", ((options[:attr_names].has_key?(value[0])) ? "#{options[:attr_names][value[0]]}" + (options[:defaults] ? " #{value[1]}" : '') : "#{value[0].humanize} #{l(value[1])}")) }),
