...
%w(rubygems open-uri active_support).each { |lib| require lib }
$:.unshift("#{File.dirname(__FILE__)}/../../../")
require 'active_support/multibyte'

begin
  ActiveSupport::Multibyte::Handlers.const_get('Codepoint')
rescue NameError
  require 'active_support/multibyte/handlers/utf8_handler_pure2'
end
...
if __FILE__ == $0
  ...
  generator.parse
  filename = File.expand_path File.dirname(__FILE__) + '/../../../active_support/values/unicode_tables.dat'
  puts "Writing to: #{filename} (#{File.size(filename)} bytes)"
